...
### W4-R5：收口全垒打 — RAG 接线 + Synthesizer 守卫 + replay/429 终态 + durable 同 key 跨进程锁 + W3.2 残余（2026-09-06）

用户决策：读三份文档（runbook / roadmap / 主日志）后确认"今天把未完成部分全部弄完" → AskUserQuestion 选 **A+B 收口全垒打** = ①文档勾选对账 + W4 真残余（RAG withRetry 接线、工具结果/Synthesizer 守卫、reconnect/replay + 上游 timeout/429 终态测试）；②durable 同 key 跨进程锁（SQLite advisory-lock 表，零外部依赖）+ W3.2 小残余（重启丢失告警、字节级磁盘治理报告）。生产改动 6 文件 + 新测试 5 文件 + 子进程 harness 1 + audit-db 扩展；零 /chat 协议/前端/FSM 改动。

- **T1（生产）RAG embedding/search withRetry 接线** —— `rag/index.js`：模块级 `OpenAIEmbeddings` 补 `maxRetries:0`（C1：withRetry = 唯一重试预算层，杜绝 SDK 默认 2 次 × withRetry 叠乘）；三条网络裸点包 `withRetry(…,{retries:2})`：首建索引 `FaissStore.fromTexts`、增量 `vectorStore.addDocuments`、请求路径 `similaritySearchWithScore`（耗尽后异常上抛 → 上传/检索调用方按失败封装）。新测试 `rag/retry.test.js`（`vi.mock` faiss 换可控假实现 + 逐 user 独立 tenant store + 共享 behavior 控制，零网络零 DB）4 用例：fromTexts flaky 一次 → calls==2；addDocuments flaky 一次 → calls==2；检索 flaky 一次 → calls==2 + ok 结果；恒 503 → calls==3（retries:2 耗尽）`classifyError` 归类 retryable `UPSTREAM_UNAVAILABLE`。
- **T2（生产）Synthesizer/融合消费者守卫** —— `chatGraph.js` 把 synthesizer 内联 `isErrorResult` 文本前缀分类器提升为模块级 `isErrorResultText` 并补结构化失败标记（`{"ok":false,…}`/`{"errorCode":…}`/`{"status":"error"|"failed"}`），4 处消费调用点改走导出函数；「未检索到相关知识片段/当前知识库为空」等"成功但空"文案保持成功侧不误伤。新测试 `chatGraphErrorResult.test.js` 4 用例：文本失败前缀→blocked、结构化降级 JSON→blocked（W4-R5 新增）、成功/空库文案→非 blocked、源级不变量（synthesizer 不再内联重复实现 + 消费点 ≥4）。
- **T3 /chat reconnect/replay 无副作用 + graph 429 终态** —— 历波幂等只到 db 层，从未在真实 HTTP 上断言"完成后再 POST 同 key → 走 `reserveChatIdempotency` completed 分支 → `writeReplaySSE`（`X-Idempotency-Replayed:true`）提前 return，不再调 executor、不再落第二条消息"。新测试 `routes/chatIdempotentReplay.test.js`（真实 DB + `createApp({getAgentExecutor: 计数 executor})` + `USE_LANGGRAPH=false` + 真实 HTTP）2 用例：首次执行一次（executorCalls==1、body 含文本+`[DONE]`、roles `[user,assistant]`）；同 key 二次 POST → replay header=true、executorCalls 仍 1、roles 不变（无重复落库）。`services/chatGraphToolReliability.test.js` 增补第 4 用例：恒 429（上游限流，与 503 同族可重试）真实 searchAgentNode × `web_search` → calls==2 耗尽、SSE `tool_error` 固定通用文案、`{ok:false,联网检索暂时不可用}` 进 search LLM input 降级、solo 仍回答、无 secret（镜像恒 503 用例）。
- **T4（生产）durable 同 key 跨进程锁 —— W4-R5-S1（闭合 §18/§19/§22/§23 残留的"需分布式锁服务"项）** —— 关键洞察：SQLite 只串行化**短写事务**，而 merge 临界区（拼装 + FAISS embed/index + settle）是**长文件工作**，单事务拿不到跨进程互斥；历波 in-process `withUploadLock` 只在单进程内保证同 key 串行，多实例/重启后两进程可同时 merge 同 hash → 双份索引/双 settle。方案：**SQLite 租约表 `upload_key_locks`**（`db/index.js` additive DDL，owner/tenant/upload_key PK + `holder_token`/`expires_at`，FK users，ledger 记 `W4-R5-S1`）。`uploadQuotaStore.js` 新 `withDurableUploadLock` 替换 durable adapter 的 `withUploadLock: withMemoryUploadLock`（内存 adapter 路径不变）：acquire = `INSERT OR IGNORE` 赢 key，冲突则**单条条件 `UPDATE` 原子回收过期租约**（`expires_at <= now`，SQLite 写串行化 → 并发回收单胜）；持有期 `ttl/3` 定时续约（慢 merge 不丢锁）；退出按 `holder_token` 删除（绝不误删他人已回收租约）；bounded busy-wait 到期抛 `UPLOAD_LOCK_TIMEOUT`(429)。崩溃持有者租约到期即被回收，key 永不卡死。新 harness `scripts/quota-lock-worker.mjs`（hold/wait 两模式）+ 新验收 `services/uploadQuotaLockDurable.test.js` 4 用例：①双 OS 进程互斥 —— A(hold 5s) 持锁期间 B 独立进程 busy 1.5s 超时（`UPLOAD_LOCK_TIMEOUT`）、A 的租约行在 DB 真实落库且未被 B 偷走、A 释放后行删除、C 立即可再拿（无卡死）；②SIGKILL 崩溃持有者 —— 租约过期 2.6s 后被新进程单条 UPDATE 回收（key 不 wedge）；③活租约不被偷（busy 超时而非抢未来行）；④过期租约原子回收。
- **T5（生产）W3.2 小残余 —— 重启丢失告警 + 字节级磁盘治理报告** —— `app.js` `startServer` 在 `initDB()` 后调新 `warnVolatileRuntimeState()`：`DURABLE_UPLOAD_QUOTA!=="true"` 上传配额内存态告警、RAG 向量进程内无落盘重载告警（持久化 RAG 指向 roadmap W6）、图片内存态(TTL 30min)告警 —— 把"重启丢什么"从文档写进真实启动日志。`scripts/audit-db.mjs`（保持只读 `readonly+fileMustExist`，零写）扩展 `quotaGovernance`（per-owner committed/reserved 字节、totals、activeReservationBytes、`upload_key_locks` lockLeases 含 active/stale）+ `diskStaging`（`./tmp`/chunks 树字节）—— 治理报告可在字节层面核对账本与磁盘。新护栏 `services/startupVolatility.test.js` 4 用例（纯源级 readFileSync）：startServer 在 initDB 后调用告警函数、告警如实覆盖三内存态、audit-db 只读打开无 `.run(`、报告含字节治理段。
- **验证**：`node --check` 各改动生产/测试/脚本 OK；定向 quota 4 组 19/19 → startupVolatility 4/4 → 全量 backend **48 files / 576 tests 全绿**（原 43/557，+5/+19）；frontend 9 files / 155 tests 保险复跑全绿（本波未改前端）；`git diff --check` exit=0（仅预期 LF/CRLF 警告）；audit-db 在 dev 库**只读 scratch 副本**上 exit=0（integrity ok，dev `agent_data.db` 本体不被触碰）。
- **残余/延后（如实）**：W4 全部明确残余清零，durable 同 key 跨进程锁**已闭合**（§18/§19/§22/§23 历史残余行该项被 §24 取代）。仍延后：软点→404（API 行为变更，FE 兼容优先）、组织 RBAC、多实例 RAG（W5/W6）、W7/W8/P2、真实可信代理链部署验证。runbook 本次无新增待办（§6 DoD 全勾 + §7 已含真实 `--apply` 执行记录）。
