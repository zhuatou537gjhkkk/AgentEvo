...
function CodeRenderer({ inline, className, children, ...props }) {
    ...
    if (inline) {
        return (
            <code
                className="rounded-md bg-[var(--panel-soft)] px-1.5 py-0.5 font-mono text-[0.92em] text-[var(--text-main)]"
                {...props}
            >
                {children}
            </code>
        ...
        <div className="code-block my-3 overflow-hidden rounded-xl shadow-sm">
            ...
            <div className="overflow-x-auto">
                {SyntaxHighlighter && theme ? (
                    ...
                    <pre className="m-0 overflow-x-auto p-3 font-mono text-[13px] leading-6 text-[var(--code-text)]">
                        <code>{codeText}</code>
                    </pre>
                )}
            </div>
                    ...
                    return (
                        ...
                        >
                            <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
                                <span className="rounded-full bg-[var(--panel-soft)] px-2 py-0.5 text-[var(--text-muted)]">{item.time || '未知时间'}</span>
                                <span
                                    className={[
                                        'rounded px-2 py-0.5',
                                        isVerified
                            ...
                            <h4 className="mb-1 text-sm font-semibold text-[var(--text-main)]">{item.title || '无标题'}</h4>
                            <p className="mb-2 whitespace-pre-wrap text-xs text-[var(--text-muted)]">{item.summary || '无摘要'}</p>
                            ...
                            {item.link && item.link !== '无链接' ? (
                                ...
                                <span className="text-xs text-[var(--text-muted)]">无可用链接</span>
                            )}

                            {item.query && (
                                <p className="mt-2 text-[11px] text-[var(--text-muted)]">检索词: {item.query}</p>
                            )}
                        </article>
                    );
                })}
        ...
        return (
            <ReactMarkdown
                ...
                components={{
                    ...
                    table({ children }) {
                        return (
                            <div className="surface-subtle my-3 overflow-x-auto rounded-lg">
                    ...
                    thead({ children }) {
                        return <thead className="bg-[var(--panel-soft)]">{children}</thead>;
                    },
                    tr({ children }) {
                        return <tr className="border-t border-[var(--panel-border)]">{children}</tr>;
                    },
                    th({ children }) {
                        return (
                            <th className="whitespace-nowrap border border-[var(--panel-border)] px-3 py-2 font-semibold text-[var(--text-main)]">
                                {children}
                            </th>
                        );
                    },
                    td({ children }) {
                        return (
                            <td className="border border-[var(--panel-border)] px-3 py-2 align-top text-[var(--text-muted)]">
                                {children}
                            </td>
                        );
                    },
    ...
    return (
        <div className={`flex w-full px-1 py-3 sm:px-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
            <div className={`flex w-full gap-3 ${isUser ? 'max-w-[82%] flex-row-reverse sm:max-w-[76%] lg:max-w-[70%]' : 'max-w-full'}`}>
                ...
                >
                    <p className={`mb-2 text-[11px] font-semibold uppercase tracking-[0.08em] ${isUser ? 'text-white/80' : 'text-[var(--text-muted)]'}`}>
                        ...
                        isEditing ? (
                            <div className="space-y-2">
                                <textarea
                                    ...
                                    className="ui-textarea w-full min-h-24 px-2 py-1.5 text-sm outline-none"
                                />
                                <div className="flex gap-2 text-xs">
                                    <button
                                        type="button"
                                        onClick={() => setIsEditing(false)}
                                        className="ui-button-ghost px-2 py-1 text-xs"
                                    >
                                        取消
                                    </button>
                                    <button
                                        ...
                                        className="ui-button-primary px-2 py-1 text-xs disabled:opacity-60"
                                    >
                                        保存并重发
                                    </button>
                                </div>
