 ...
 *   - GRAPH_DAG_SCHEDULER_ENABLED=true → planner 走 R3 依赖校验，fanout 进 dag_scheduler；
 ...
 */
function makeDagFake({ knowledgeOutcome = "ok" } = {}) {
    ...
    const invoke = async (msgs) => {
        ...
        if (joined.includes("任务规划助手")) return { content: PLAN_JSON };
        // knowledge 子任务节点（plan 并行态 firstResponse invoke）→ 检索结果
        if (joined.includes("知识库检索专家")) {
            return { content: knowledgeOutcome === "ok" ? KB_HIT : KB_ERR };
        }
    ...
    return {
        ...
        log,
        callsFor,
        knowledgeCalls: () => callsFor("知识库检索专家"),
